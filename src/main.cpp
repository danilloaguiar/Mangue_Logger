...
const int warning = PA0;
const int logging = PA1;
const int freq_channel1 = PB5;
const int freq_channel2 = PB6;
const int pot0 = PA3;
const int pot1 = PA4;
const int pot2 = PA5;
const int start = PB4;
...
void setup() 
{
  ...
  pinMode(warning, OUTPUT);                          // When device is ready, led is permanently OFF
  ...
  myFile = SD.open(name_file, FILE_WRITE);                            // Creates first data file
  uint32_t t1 = millis();                                             // Start device timer
  attachInterrupt(freq_channel1, freq_channel1_ISR, FALLING);
  attachInterrupt(freq_channel2, freq_channel2_ISR, FALLING);
  Ticker acq(&sampleISR, 1.0/SAMPLE_FREQ);                            // Start data acquisition
  ...
  while(running)
    {
        ...
        {   
          ...
          buffer.pop();                  
          buffer_counter--;
          myFile.write((const uint8_t*)&temp,sizeof(packet_t));
          svd_pck++;
        ...
        if((millis()-t1 > 10) && (millis()-t1 < 500))
          attachInterrupt(start,toggle_logging,FALLING);
    }
...
{
  running = !running;
  attachInterrupt(start,NULL,FALLING);
}
